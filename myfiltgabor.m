...
xlabel('m');
ylabel('|C(m,n)|');
title('Time Section of Gabor Distribution for SP500');
hold on;
li(1:m)=spmask;
...
xlabel('m');
ylabel('|C(m,n)|');
title('Time Section of Gabor Distribution for NASDAQ');
hold on;
li(1:m)=nasmask;
p1=plot(li,'LineWidth',6,'Color','b');
legend(p1,'Threshold');
...
figure;
    ...
    h=get(gca,'xlabel');
    set(h,'rotation',30)
    ylabel('Frequency','FontSize',12,'FontWeight','bold','Color','b')
    h=get(gca,'ylabel');
